...
import Table from "../InteractiveContent/Table/Table"; // ✅ Додаємо новий компонент Table
...
const ContentRenderer = ({ sections, allSectionsWithIds, parentIndex = [], level = 2 }) => {
  ...
  return sections.map((section, index) => {
    ...
    switch (section.type) {
      ...
      case "table": // ✅ Додаємо новий випадок для компонента Table
        return (
          <SectionContainer key={index}>
            {hasTitle && (
              <SectionHeading as={`h${sectionHeadingLevel}`} size="default" id={sectionId}>
            ...
            <Table tableTitle={section.tableTitle} headers={section.headers} rows={section.rows} />
          </SectionContainer>
        );
